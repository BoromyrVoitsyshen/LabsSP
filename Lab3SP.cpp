...
std::string identifyNumber(const std::string& str) {
...
std::string identifyConstant(const std::string& str) {
...
void parse(const std::string& str) {
    ...
    while (i < len) {
        if (str[i] == '#') {  // Директива препроцесора
            ...
            while (i < len && !isDelimiter(str[i])) directive += str[i++];
            ...
            std::cout << "'" << number << "' IS " << identifyNumber(number) << "\n";
            ...
            constant += str[i++];
            std::cout << "'" << constant << "' IS " << identifyConstant(constant) << "\n";
...
// Головна функція
int main() {
    std::string code = "#define MAX 100 //Some max value\nint main(){\nstd::string STRING = \"Hello World\";\nint INT = 2024;\nfloat FLOAT = 3.14;\nchar CHAR = \'A\';}";
    parse(code);
    return 0;
}